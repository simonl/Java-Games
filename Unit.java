...
public class UnitsQ {
	...
	public static final UnitsQ JOULES = new UnitsQ(2, -2, 1, 0, 0, 0, 0);
	...
	public UnitsQ(int L, int T, int M, int C, int P, int Te, int A) {
	...
	public UnitsQ plus(UnitsQ u) {
		Unit[] result = new Unit[7];

		for(int i : range(7))
			result[i] = this.units[i].plus(u.units[i]);

		return new UnitsQ(result);
	}

	public UnitsQ minus(UnitsQ u) {
		Unit[] result = new Unit[7];

		for(int i : range(7))
			result[i] = this.units[i].minus(u.units[i]);
	...
	public UnitsQ plus(UnitsQ u) {
		...
		for(int i : range(7))
			result[i] = this.units[i].divide(u.units[i]);
