...
public class Matrix
{
    ...
    //Elimination
    public Matrix getGaussianElimination()
    {
	Matrix aGaussian = copy();
	    
	    aGaussian.setToGaussianElimination();
	    
	return aGaussian;
    }
    
    public void setToGaussianElimination()
    {   
	int other = 2;
	
	for(int i = 1; i <= getM(); i ++)
	{
	    if(getCoefficient(i, i) != 0.0)
	    {
		multiplyRow(i, 1.0/getCoefficient (i, i));
		
		for(int h = i + 1; h <= getM(); h ++)
		    if(getCoefficient(h, i) != 0.0)
			addMultipleOfRow(h, i, -getCoefficient(h, i));
		    
		other = i + 2;
	    }
	    else
	    {
		exchangeRows(i, other);
		i --;
		other ++;
	    }
    ...
    public Matrix getJordanElimination()
    {
	Matrix aJordan = copy();
	
	    aJordan.setToJordanElimination();
	    
	return aJordan;
    }
    
    public void setToJordanElimination()
    {
	int other = getM() - 1;
    ...
    public void setToJordanEliminationFromGaussian()
    {
	for(int i = getM(); i > 0; i --)
	    for(int h = i - 1; h > 0; h --)
		if(getCoefficient(h, i) != 0.0)
    ...
    public Matrix getGaussJordanElimination()
    {
	Matrix aGaussJordan = copy();
	
	    aGaussJordan.setToGaussJordanElimination();
	    
	return aGaussJordan;
    }
    
    public void setToGaussJordanElimination()
    {
	setToGaussianElimination();
	setToJordanEliminationFromGaussian();
    }
