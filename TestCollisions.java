...
public class TestCollisions
{
	...
	{            
	    ...
	    final double G = 0;
	    Vector gravity = new CartesianVector(0, G, 0);
		...
		int posY = 150 * ((i % 16) / 4 + 1);
		    int posZ = 150 * (i / 16 + 1);
		balls[i].motion = new CoordinateSystem(new CartesianVector(posX, posY, posZ), randomCartesianVector(100, 100, 100));
	    }
		...
		Vector momentum = new Vector(), wallMomentum = new Vector();
		    ...
		    window.assignConstants(kineticE, potentialE, momentum.getMagnitude());
		    window.setConstants();
		...
		if(Math.random() > 0.9999)
		{
		    ...
		    window.assignConstants(kineticE, potentialE, momentum.addVector(wallMomentum).getMagnitude());
		}
		
		    
		for(int i = 0; i < num; i ++)
		{
		    balls[i].updateParticle(gravity.scalarProduct(balls[i].getMass()));
		}
