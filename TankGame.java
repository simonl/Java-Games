...
public class TankGame
{
	...
	public static void checkForImpacts(Tank tank1, Tank tank2)
	{
/*
...
*/
		...
		checkBalls(tank1.ball1, tank1.ball2, tank2.ball1, tank2.ball2);
...
*/
		...
		for(Tank tank : tanks ) {
			tank.ball1 = checkImpact(tank.ball1, tanks);
			tank.ball2 = checkImpact(tank.ball2, tanks);
		}
	}


	public static void checkTanks(Tank... tanks) {
		for(Tank tank : tanks)
			checkTankForWall(tank);

		for(int i = 0; i < tanks.length; i++)
			for(int j = i+1; j < tanks.length; j++)
				checkForCrash(tanks[i], tanks[j]);
	}

	public static void checkBalls(Cannonball... balls) {
		for(Cannonball ball : balls)
			checkBallForWall(ball);

		for(int i = 0; i < balls.length; i++)
			for(int j = i+1; j < balls.length; j++)
				checkBallForBall(balls[i], balls[j]);
	}

	public static Cannonball checkImpact(Cannonball ball, Tank... tanks) {
		if(ball != null && ball.checkIfLanded()) {
			for(Tank tank : tanks)
				tank.getHit(ball.finalPosition);
			return null;
		}
	...
	public static void checkBallForBall(Cannonball ball1, Cannonball ball2)
	{
		...
		if(ball1.coordinates.getProximity(ball2.coordinates) <= Cannonball.ballDimensions.getHeight()/2.0)
		{
			ball1.explode();
			ball2.explode();
...
//		ball.finalPosition.setPositionY(ball.coordinates.getPositionY());
	...
	public static void checkForCrash(Tank tank1, Tank tank2)
	{
		...
		{
		    tank1.crash(speed, middle - 15.0);
		    tank2.crash(speed, middle + 15.0);
		}
		else
		{
		    tank1.crash(speed, middle + 15.0);
